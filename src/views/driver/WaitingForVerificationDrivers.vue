      ...
      <vx-card>
        <div id="data-list-list-view" class="data-list-container">
          <vs-table
            ...
            search
            :data="WaitingForVerificationDrivers"
          >
            <div
              slot="header"
            ...
            <template slot-scope="{ data }">
              <tbody>
                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                  ...
                  <vs-td>
                    <p class="product-category">
                      {{ tr.RegistrationDate | formatDate("yyyy/MM/dd HH:mm") }}
                    </p>
                  </vs-td>
...
export default {
  ...
  methods: {
    ...
    Search(value) {
      ...
      this.WaitingForVerificationDrivers = [];
      this.$store
        .dispatch("DriverList/WaitingForVerificationDrivers", this.Model)
        .then(res => {
          if (res.status == 200) {
            if (res.data.length > 0) {
              this.WaitingForVerificationDrivers = res.data;
