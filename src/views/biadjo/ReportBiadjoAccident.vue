<template>
  <div>
    <questionnaire @clicked="onClickChildSubmit"></questionnaire>
  </div>
</template>
...
export default {
  ...
  computed: {},
  methods: {
    onClickChildSubmit(value) {
      this.$vs.loading();

      let QuestionnaireString =
        "{" +
        '"' +
        this.$t(value.Question1) +
        '"' +
        ":" +
        '"' +
        value.Answer1 +
      ...
      this.Model.Questionnaire = QuestionnaireString;
      this.$store
        .dispatch("BiadjoList/ReportAccidentBiadjo", this.Model)
        .then(res => {
  ...
  created() {
    ...
    this.Model.BiadjoID = this.$route.params.Id;
