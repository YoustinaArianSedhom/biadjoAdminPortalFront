...
export default {
  ...
  methods: {
    Activepopup(id) {
      this.ModelAddnIsurance.BiadjoId = id;
      this.popupActiveAddInsurance = true;
    },
    AddInsurancePolictToBiadjo(ModelAddnIsurance) {
      this.$vs.loading();
      this.$store
        .dispatch("BiadjoList/AddInsurancePolictToBiadjo", ModelAddnIsurance)
        .then(response => {
    ...
    Search(value) {
      ...
      this.$store
        .dispatch("BiadjoList/GetBiadjosNeedInsurance", this.BiadjoModel)
        .then(response => {
          if (response.status == 200) {
