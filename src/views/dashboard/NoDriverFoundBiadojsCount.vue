...
export default {
  data() {
    return {
      ...
      option: {
        title: {
          text: this.$t("NoDriverFoundBiadojsCount"),
  ...
  methods: {
    NoDriverFoundBiadojsCountComponent() {
      var Obj = {};
      ...
      if (this.$store.state.SelectedCountryCode) {
        this.$store
          .dispatch("Dashboard/NoDriverFoundBiadojsCount", Obj)
          .then(response => {
    ...
    DashboardReloading() {
      this.clearSetInterval = setInterval(() => {
        this.NoDriverFoundBiadojsCountComponent();
  ...
  created() {
    ...
    this.NoDriverFoundBiadojsCountComponent();
    window.NoDriverFoundBiadojsCountComponent = this.NoDriverFoundBiadojsCountComponent;
  },
  beforeDestroy() {
    this.StopInterval();
  }
