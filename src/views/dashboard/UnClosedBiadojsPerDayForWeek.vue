...
export default {
  data() {
    return {
      ...
      option: {
        title: {
          text: this.$t("UnClosedBiadojsPerDayForWeek"),
  ...
  methods: {
    UnClosedBiadojsPerDayForWeekComponent() {
      var Obj = {};
      ...
      if (this.$store.state.SelectedCountryCode) {
        this.$store
          .dispatch("Dashboard/UnClosedBiadojsPerDayForWeek", Obj)
          .then(response => {
    ...
    DashboardReloading() {
      this.clearSetInterval = setInterval(() => {
        this.UnClosedBiadojsPerDayForWeekComponent();
  ...
  created() {
    ...
    this.UnClosedBiadojsPerDayForWeekComponent();
    window.UnClosedBiadojsPerDayForWeekComponent = this.UnClosedBiadojsPerDayForWeekComponent;
  },
  beforeDestroy() {
    this.StopInterval();
  }
